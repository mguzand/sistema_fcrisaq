			...
			$errors[] = "ID vacío";
			 ...
			 !empty($_POST['user_id_mod']) 
				...
				$objeto=" PANTALLA LIBRO DIARIO";
                     $us="probando";
                    $accion="SE ELIMINO UN REGISTRO";
					
               
					// write new user's data into database
				//	$sql = "UPDATE  tbl_libro_diario
				//	set status = 'ELIM'
				//	WHERE id_libro_diario='".$id_parametro."'";
				//	$query = mysqli_query($mysqli,$sql);
				//	
					$sql = "DELETE  FROM tbl_libro_diario WHERE id_libro_diario='".$id_parametro."'";
					$query = mysqli_query($mysqli,$sql);
					
 
					// if user has been added successfully
					//var_dump($id_parametro);
		if ($query) {
					$bita=grabarBitacora($idUsuario,$objeto,$accion,$sql);
                        $messages[] = "un registro ha sido eliminado con éxito.";
                    } else {
