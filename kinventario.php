...
//var_dump($insertar);
	...
	$insertar=getPer('permiso_insercion',$rol,'18');

echo $insertar;
		$objeto="PANTALLA KARDEX";
		$accion="INGRESO A LA PANTALLA KARDEX";
		$descripcion="ingreso a pantalla Kardex";
	...
	<div class="contact-top">
		...
		<?php } ?>
			...
			<form class="form-horizontal" role="form" id="datos_cotizacion">
                ...
                include("modal/eliminar_inventario.php");
...
$( "#editar_password" ).submit(function( event ) {
 ...
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/eliminar_inventario.php",
			data: parametros,
...
$( "#editar_usuario" ).submit(function( event ) {
 ...
 var parametros = $(this).serialize();
  ...
  event.preventDefault();
		...
		function obtener_datos(id,codigo,equipo,cant,tp,proveedor){ 				
			$("#mod").val(id);
			$("#cod").val(codigo);
			$("#eq").val(equipo);
			$("#cant").val(cant);	
...
$('#bd-desde').on('change', function(){
		...
		var url = 'ajax/buscar_inventario_fecha.php';
		$.ajax({
		type:'POST',
		url:url,
...
function reportePDF(){
	var desde = $('#bd-desde').val();
	var hasta = $('#bd-hasta').val();
	window.open('reporte/rinventario.php?desde='+desde+'&hasta='+hasta);
}
